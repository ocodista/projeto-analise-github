...
import type { Repository, CommitWeek, SearchResponse, OpenPRCountResponse } from "../types/github";
...
export function fetchOpenPRCount(fullName: string): Promise<number> {
  ...
  return githubFetch<OpenPRCountResponse>(`/search/issues?q=${query}&per_page=1`).then(
...
export function fetchContributorsCount(fullName: string): Promise<number> {
  return githubFetchWithLinkCount(`/repos/${fullName}/contributors?per_page=1&anon=true`);
}

export function searchRepositories(query: string): Promise<SearchResponse> {
  const encoded = encodeURIComponent(query);
  return githubFetch<SearchResponse>(`/search/repositories?q=${encoded}&per_page=10&sort=stars`);
}
